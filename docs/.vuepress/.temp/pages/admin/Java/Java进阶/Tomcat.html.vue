<template><div><h2 id="tomcat" tabindex="-1"><a class="header-anchor" href="#tomcat"><span>Tomcat</span></a></h2>
<blockquote>
...
<h3 id="开启后出现闪退的几种问题" tabindex="-1"><a class="header-anchor" href="#开启后出现闪退的几种问题"><span>开启后出现闪退的几种问题</span></a></h3>
<ol>
...
<h3 id="关闭" tabindex="-1"><a class="header-anchor" href="#关闭"><span>关闭</span></a></h3>
<p>双击 bin\shutdown.bat</p>
<h3 id="tomcat-目录结构" tabindex="-1"><a class="header-anchor" href="#tomcat-目录结构"><span>Tomcat 目录结构</span></a></h3>
<ol>
<li>bin：存放 Tomcat 的启动和关闭的脚本文件</li>
<li>conf：存放 Tomcat 的配置文件</li>
<li>lib：存放 Tomcat 的依赖 jar 包</li>
<li>logs：存放 Tomcat 的日志文件</li>
<li>temp：存放 Tomcat 的临时文件</li>
<li>webapps：存放部署的 Web 项目</li>
<li>work：存放 Tomcat 运行时编译的文件</li>
</ol>
<h3 id="web-项目目录结构" tabindex="-1"><a class="header-anchor" href="#web-项目目录结构"><span>Web 项目目录结构</span></a></h3>
<ol>
<li>html</li>
<li>css</li>
<li>js</li>
<li>META-INF：存放项目的描述文件</li>
<li>WEB-INF：存放项目的配置文件
<ul>
<li>web.xml：项目的核心配置文件</li>
<li>classes：存放项目的字节码文件</li>
<li>lib：存放项目的依赖 jar 包</li>
</ul>
</li>
<li>index.jsp：项目的首页</li>
</ol>
<h3 id="tomcat-内部机制" tabindex="-1"><a class="header-anchor" href="#tomcat-内部机制"><span>Tomcat 内部机制</span></a></h3>
<p>了解内部机制，才能知道 tomcat 相关的配置和 server.xml 文件的作用
<img src="@source/admin/Java/Java进阶/images/Tomcat内部机制.jpg" alt="Tomcat内部机制"></p>
<h3 id="部署-web-项目" tabindex="-1"><a class="header-anchor" href="#部署-web-项目"><span>部署 Web 项目</span></a></h3>
<h4 id="方式一" tabindex="-1"><a class="header-anchor" href="#方式一"><span>方式一</span></a></h4>
<ol>
<li>将 web 项目复制到 Tomcat 的 webapps 目录下</li>
<li>启动 Tomcat</li>
<li>访问项目：http://localhost:8080/项目名/资源路径</li>
<li>localhost:8080/web-app/a.html</li>
</ol>
<ul>
<li>注意：如果项目名是 ROOT，访问时是不需要写 ROOT 的。</li>
</ul>
<h4 id="方式二" tabindex="-1"><a class="header-anchor" href="#方式二"><span>方式二</span></a></h4>
<ol>
<li>在 Tomcat 的 conf 中的 server.xml 文件中配置项目</li>
<li>启动 Tomcat</li>
<li>访问项目：http://localhost:8080/项目名/资源路径</li>
</ol>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!--
  path: 访问项目的虚拟目录
  docBase: 项目存放的全路径
  注意：
    1. 路径不能使用相对路径
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="方式三" tabindex="-1"><a class="header-anchor" href="#方式三"><span>方式三</span></a></h4>
<ol>
<li>在 Tomcat 的 conf\catalina\localhost 目录下创建一个 xml 文件</li>
<li>在 xml 文件中配置项目</li>
<li>启动 Tomcat</li>
<li>访问项目：http://localhost:8080/项目名/资源路径</li>
</ol>
...
<ol>
<li>与方式二类似，但是比方式二更方便灵活，不用反复启动 tomcat</li>
<li>不要 path 来指定虚拟目录，而把项目名就当作是虚拟目录</li>
</ol>
<h4 id="方式四" tabindex="-1"><a class="header-anchor" href="#方式四"><span>方式四</span></a></h4>
<ol>
<li>将 Web 项目打成 war 包</li>
<li>将 war 包复制到 Tomcat 的 webapps 目录下，会自动解压</li>
<li>启动 Tomcat</li>
<li>访问项目：http://localhost:8080/项目名/资源路径</li>
</ol>
<h3 id="本地域名设置" tabindex="-1"><a class="header-anchor" href="#本地域名设置"><span>本地域名设置</span></a></h3>
<ol>
<li>在 C:\Windows\System32\drivers\etc\hosts 文件中添加</li>
</ol>
<div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre v-pre class="language-text"><code>127.0.0.1   www.aaa.com
...
<li>在 bin 目录统计结构处新建 aaa_webapp 和 bbb_app</li>
<li>将项目分别复制到 aaa_webapp 和 bbb_app 中</li>
<li>访问项目：http://www.aaa.com:8080/项目名/a.html</li>
<li>访问项目：http://www.bbb.com:8080/项目名/b.html</li>
</ol>
<ul>
<li>注意：</li>
</ul>
<ol>
<li>端口号 8080 可以在 server.xml 中自由配置</li>
<li>如果端口号被改成 80 了，则访问时就不需要加端口号了，因为 80 是 Tomcat 的默认端口号</li>
<li>如果项目名是 ROOT，则访问时就不需要写项目名了</li>
<li>如果访问的文件是 index.html，则访问时就不需要写文件名了</li>
<li>最终访问可能就像www.aaa.com(www.baidu.com)一样就可以访问项目了</li>
</ol>
<h3 id="在-idea-中集成-tomcat" tabindex="-1"><a class="header-anchor" href="#在-idea-中集成-tomcat"><span>在 idea 中集成 Tomcat</span></a></h3>
</div></template>
