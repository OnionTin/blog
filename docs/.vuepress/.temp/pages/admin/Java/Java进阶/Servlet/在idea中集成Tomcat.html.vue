<template><div><h2 id="在-idea-中集成-tomcat" tabindex="-1"><a class="header-anchor" href="#在-idea-中集成-tomcat"><span>在 idea 中集成 Tomcat</span></a></h2>
<ol>
<li>创建新的 Web 项目
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果是 Gradle 项目，在 build.gradle 中添加：</p>
<div class="language-groovy line-numbers-mode" data-ext="groovy" data-title="groovy"><pre v-pre class="language-groovy"><code>dependencies <span class="token punctuation">{</span>
...
<p>创建 Servlet 类</p>
<ul>
<li>在 src\main\java 目录下，右键你的包名，选择 New &gt; Servlet（如果看不到 Servlet 选项，确保你选择了正确的目录层级，或者直接创建一个普通的 Java 类）。命名你的 Servlet（例如 HelloWorldServlet），并实现逻辑。</li>
</ul>
</li>
<li>
<p>配置 web.xml -在 WEB-INF 目录下的 web.xml 中，配置 Servlet 映射：</p>
</li>
</ol>
...
<p>部署并运行项目</p>
<ul>
<li>右键项目，选择 Run &gt; Edit Configurations。</li>
<li>点击左上角的+，选择 Tomcat Server &gt; Local。</li>
<li>在新出现的配置窗口中，选择你刚才创建的项目作为 Deployment。</li>
<li>点击 Apply，然后 OK。</li>
<li>使用菜单栏的 Run &gt; Run 'MyWebApp'来启动 Tomcat 服务器。</li>
</ul>
</li>
<li>
<p>访问 Servlet</p>
<ul>
<li>打开浏览器，输入 http://localhost:8080/your-context-path/hello（your-context-path 通常是项目名，除非你在部署时指定了不同的上下文路径），</li>
<li>你应该能看到 Servlet 的响应。</li>
<li>这样，你就成功地在 IntelliJ IDEA 中创建了一个 Web 项目，并通过 Tomcat 服务器访问了 Servlet。</li>
</ul>
</li>
<li>
<ul>
...
<li>
<ol>
<li>选择到 Tomcat 的安装根目录，而不是 bin 目录。Tomcat 的根目录通常包含了 bin、conf、lib 等多个子目录。</li>
</ol>
</li>
<li>
<ol start="2">
<li>快捷键 ctrl + alt + s 快速打开配置窗口</li>
</ol>
</li>
</ul>
</div></template>
