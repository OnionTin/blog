<template><div><h2 id="溢出属性检查" tabindex="-1"><a class="header-anchor" href="#溢出属性检查"><span>溢出属性检查</span></a></h2>
<blockquote>
...
<div class="language-typescript line-numbers-mode" data-ext="ts" data-title="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">interface</span> <span class="token class-name">SquareConfig</span> <span class="token punctuation">{</span>
...
<p>在这个例子中，对象字面量 { colour: &quot;red&quot;, width: 100 } 包含了一个 SquareConfig 接口中未定义的属性 colour，所以 TypeScript 编译器会报错，指出 'colour' does not exist in type 'SquareConfig'
。</p>
</blockquote>
<h3 id="要解决这个问题-你可以做以下几件事情之一" tabindex="-1"><a class="header-anchor" href="#要解决这个问题-你可以做以下几件事情之一"><span>要解决这个问题，你可以做以下几件事情之一：</span></a></h3>
<ol>
<li>修正属性名，确保对象字面量中的属性名称与目标类型中的属性名称相匹配。</li>
<li>如果确实需要传递额外的属性，可以修改目标类型的定义，以包含这些额外的属性。</li>
<li>使用类型断言，将对象字面量断言为任意类型，来绕过溢出属性检查（但这种做法会失去类型安全）。</li>
</ol>
</div></template>
